...
def generar_datos():
    ...
    df_dim_alumnos = pd.DataFrame(
        {
            ...
            "Edad": edades,
            "Aula": np.random.choice(["101", "102", "201", "202"], 
                                     size=len(alumnos)),
        }
    )
    ...
    df_fact_notas = pd.merge(df_fact_notas, 
                             df_dim_cursos["Id_Curso"], 
                             how="cross")
    df_fact_notas["Id_Nota"] = np.arange(1, 
                                         len(df_fact_notas["Id_Fecha"]) + 1)
    df_fact_notas["Nota"] = np.clip(
        np.random.normal(loc=13.2, 
                         scale=3.4, 
                         size=len(df_fact_notas)), 
                         3, 
                         20
    )
    df_fact_notas.loc[df_fact_notas["Nota"] < 5, "Nota"] = np.nan
    df_fact_notas.rename(
        columns={"Id_Curso": "IdCurs", 
                 "Id_Alumno": "Id_A"}, 
                 inplace=True
    )

    return df_dim_calendario, df_dim_alumnos, df_dim_cursos, df_fact_notas


df_dim_calendario, df_dim_alumnos, df_dim_cursos, df_fact_notas = generar_datos()


# Hacer merge
@st.cache_data
def hacer_merge(alumnos, cursos, notas):
    return pd.merge(notas, alumnos, 
                    left_on="Id_A", 
                    right_on="Id_Alumno").merge(
        cursos, 
        left_on="IdCurs", 
        right_on="Id_Curso"
    )
...
df_merge = hacer_merge(df_dim_alumnos, df_dim_cursos, df_fact_notas)
...
with st.expander("Click para ver contenido"):
    ...
    def limpieza_datos():
        ...
        df_copia = df_fact_notas.copy()
        df_copia["Nota"] = df_copia["Nota"].fillna(5)
        ...
        return df_copia
    df_fact_notas2 = limpieza_datos()
    st.dataframe(df_fact_notas2.head(20))
    st.code(f"{inspect.getsource(limpieza_datos)}",
            language="python")
...
with st.expander("Click para ver el contenido"):
    ...
    df_cursos_ag = df_fact_notas2.groupby("Id_Curso").agg({"Nota": "mean"})
    ...
    df_cursosalumnos_ag = df_fact_notas2.groupby(["Id_Alumno", "Id_Curso"]).agg(
        {"Nota": "mean"}
    )
...
with st.expander("Click para ver el contenido"):
    ...
    crear_histplot()
    st.code(f"""{inspect.getsource(crear_histplot)}""")
...
with st.expander("Click para ver el contenido"):
    ...
    crear_boxviolinplot()
    st.code(f"{inspect.getsource(crear_boxviolinplot)}",language="python")
...
with st.expander("Click para ver el contenido"):
    ...
    crear_heatmap()
    st.code(f"{inspect.getsource(crear_heatmap)}",language="python")
...
with st.expander("Click para ver el contenido"):
    ...
    crear_lineplot()
    st.code(f"{inspect.getsource(crear_lineplot)}",language="python")
...
with st.expander("Click para ver el contenido"):
    ...
    crear_barplot()
    st.code(f"{inspect.getsource(crear_barplot)}",language="python")
...
with st.expander("Click para ver el contenido"):
    ...
    crear_regplot()
    st.code(f"{inspect.getsource(crear_regplot)}",language="python")
...
with st.expander("Click para ver el contenido"):
    ...
    def crear_countplot():
        aulas_selecionadas3 = st.multiselect(
            ...
            options=aulas, 
            default=aulas, 
            key="Filtro_countplot"
        )

        if aulas_selecionadas3:
            df_merge["Estado"] = np.where(
                df_merge["Nota"] >= 13, 
                "Aprobado", 
                "Desaprobado"
            )
    ...
    crear_countplot()
    st.code(f"{inspect.getsource(crear_countplot)}",language="python")
